...
exports.adminLogin = async (req, res) => {
    ...
    try {
        const [rows] = await pool.query('SELECT id, email, password_hash, role, igreja_id, nome FROM usuarios WHERE email = ? LIMIT 1', [email]);
        const user = rows[0];
        ...
        res.json({ token, user: { id: user.id, nome: user.nome, email: user.email, role: user.role, churchId: user.igreja_id } });
    } catch (error) {
